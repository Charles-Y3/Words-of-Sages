...
// Full text from Wikisource (Traditional), paragraph-segmented.
const paragraphs = [
  `太上曰：「禍福無門，惟人自召；善惡之報，如影隨形。」`,
  `是以天地有司過之神，依人所犯輕重，以奪人算。算減則貧耗，多逢憂患；人皆惡之，刑禍隨之，吉慶避之，惡星災之；算盡則死。`,
  `又有三台、北斗神君，在人頭上，錄人罪惡，奪其紀算。又有三尸神，在人身中，每到庚申日，輒上詣天曹，言人罪過。月晦之日，竈神亦然。凡人有過，大則奪紀，小則奪算。其過大小，有數百事，欲求長生者，先須避之。`,
  ...
  `又諸橫取人財者，乃計其妻子、家口以當之，漸至死喪。若不死喪，則有水火盜賊、遺亡器物、疾病口舌諸事，以當妄取之值。又枉殺人者，是易刀兵而相殺也。取非義之財者，譬如漏脯救飢，鴆酒止渴；非不暫飽，死亦及之。`,
...
function splitLong(text) {
  ...
  let cut = text.lastIndexOf("；", mid + 40);
  if (cut < mid - 60) cut = text.lastIndexOf("。", mid + 40);
  if (cut < 0) cut = mid;
  const left = text.slice(0, cut + 1);
...
for (const p of paragraphs) {
  const parts = splitLong(p);
  parts.forEach((textZh, i) => {
    section += 1;
    const part = parts.length > 1 ? i + 1 : null;
    // Label by original paragraph groups 1..13 with optional -part
    // Simpler: sequential section labels matching reading order
    units.push({
...
fs.writeFileSync(path.join(__dirname, "_ganying-units.json"), JSON.stringify(units, null, 2));
